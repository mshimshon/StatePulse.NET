...
internal class StatePulseRegistry : IStatePulseRegistry
{
    ...
    private ConcurrentDictionary<Type, (object Instance, Guid Id, DispatchCancelToken Cts)> _tracker = new();
    ...
    public void CancelDispatch(Type action)
    {
        ...
        foreach (var tracked in actionsToCancel)
            if (tracked.Key == action && tracked.Value.Cts != null && !tracked.Value.Cts.IsCancellationRequested)
            {
                tracked.Value.Cts.Cancel();
                tracked.Value.Cts.Destroy();
                _tracker.TryRemove(tracked);
            }
    }
    ...
    public void TrackDispatch(Type action, object instance, Guid id)
    {
        _tracker.TryAdd(action, new(instance, id, new DispatchCancelToken()));
    }
